...
#else
    const char* pszModule = "nanocashcoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Nanocashcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Nanocashcoin
    // Mac: ~/Library/Application Support/Nanocashcoin
    // Unix: ~/.nanocashcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Nanocashcoin";
#else
    fs::path pathRet;
...
{
    namespace fs = boost::filesystem;
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NCCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NCCoin
    // Mac: ~/Library/Application Support/NCCoin
    // Unix: ~/.nccoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NCCoin";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "nanocashcoin.conf"));
    if (!pathConfigFile.is_complete()) pathConfigFile = GetDataDir(false) / pathConfigFile;

    // Load old config file if present
    if (mapArgs.count("-conf") == 0 && !boost::filesystem::exists(pathConfigFile)) {
        boost::filesystem::path pathOldConfigFile = GetDataDir(false) / "nccoin.conf";
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "nanocashcoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Nanocashcoin will not work properly.");
...
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments)
{
    ...
    ss << "/";
    ss << "Nanocashcoin:" << FormatVersion(NANOCASHCOIN_VERSION);
    ss << "(" << CLIENT_BUILD << ")/";
    return ss.str();
}
