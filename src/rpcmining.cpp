...
Value GetNetworkHashPS(int lookup, int height) {
    ...
    if (lookup <= 0){
		...
		int64 nTargetTimespan = 6 * nStakeTargetSpacing * 24 * 7;  // one week
	...
	double tmp = avgDiff * (2^31) / (lookup / predictedBlockCount);
...
// nccoin: get network Gh/s estimate
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Nanocashcoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Nanocashcoin is downloading blocks...");
