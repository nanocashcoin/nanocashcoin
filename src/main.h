...
static const int64 MAX_MINT_PROOF_OF_WORK = 4999 * COIN;
static const int64 MIN_TXOUT_AMOUNT = MIN_TX_FEE;
static const int64 COIN_PREMINE = 500000000 * COIN;
/** Coinbase transaction outputs can only be spent after this number of new blocks (network rule) */
static const int COINBASE_MATURITY_NCC = 50;
...
static const uint256 hashGenesisBlockOfficial("0x00000001ab95881f70c4dccc5407189955b6f2a6c7082d866c098faa1b2c879c");
static const uint256 hashGenesisBlockTestNet("0x0000000efd0ed613212ec6f4b18cb053a283b3965d56c9ff1f249c82b4dfe32c");
...
public:
    ...
    {
        // nccoin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CValidationState &state, CCoinsViewCache &view, uint64& nCoinAge) const;  // nccoin: get transaction coin age
...
public:
    ...
    bool fCoinStake;      // nccoin: if the outpoint was the last unspent: whether it belonged to a coinstake
    unsigned int nTime;   // nccoin: if the outpoint was the last unspent: its tx timestamp
...
public:
    ...
    // nccoin: whether transaction is a coinstake
    bool fCoinStake;

    // nccoin: transaction timestamp
    unsigned int nTime;
    ...
    bool IsCoinStake() const { // nccoin: coinstake
        return fCoinStake;
    }
    ...
    bool Spend(const COutPoint &out, CTxInUndo &undo) {
        ...
        if (vout.size() == 0) {
            ...
            undo.fCoinStake = fCoinStake;  // nccoin
            undo.nTime = nTime;            // nccoin
        }
        return true;
    }
...
public:
    ...
    // nccoin: block signature - signed by coin base txout[0]'s owner
    ...
    // nccoin: two types of block: proof-of-work or proof-of-stake
    ...
    // nccoin: get max transaction timestamp
    ...
    bool GetCoinAge(uint64& nCoinAge) const; // nccoin: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // nccoin: entropy bit for stake modifier if chosen by modifier
};
...
public:
    ...
    // (memory only) Total amount of trust score (nccoin proof-of-stake difficulty) in the chain up to and including this block
    uint256 nChainTrust;
    ...
    // nccoin: money supply related block index fields
    int64 nMint;
    int64 nMoneySupply;

    // nccoin: proof-of-stake related block index fields
    unsigned int nFlags;  // nccoin: block index flags
    enum
    {
