...
UNIX BUILD NOTES FOR NANOCASHCOIN
====================
...
---------------------
	...
	mkdir -p ~/.nanocashcoin
	echo "rpcuser=username" >>~/.nanocashcoin/nanocashcoin.conf
	echo "rpcpassword=`head -c 32 /dev/urandom | base64`" >>~/.nanocashcoin/nanocashcoin.conf
	git clone https://github.com/nanocashcoin/nanocashcoin
	cd nanocashcoin
	git checkout v0.6.0ncc
	qmake -qt=qt5 && make
...
Note: Currently, there is no nanocashcoin ebuild available in overlay 
...
Notes
-----
The release is built with GCC and then "strip nanocashcoind" to strip the debug
